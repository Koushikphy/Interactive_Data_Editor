 ...
 *
 * This is a jExcel Pro version - You must have a proper license to use this file.
 *
 * @Roadmap
 * Search, Pagination, Lazyloading Backend
 * Remote persistance, updates
 * Multiple border coloring
 * Marching antes border copy indication
 * Filter by column
 * Footer row to SUM
 * varias colunas selecioadas, se houver um setwidth, atualizar todas as colunas selecionasd
 * Paste está colando em colunas escondidas!
 *
 * @Improviments from 3 to 4
 * Multiple column resize
 * New key navegation
 * New pagination/lazyloading up to 20x faster
 * Big data support
 * Superevent isso 'e para ajudar com o remote collaboration
 * quando usando nestedHead. oninsertcolumn... atualizar data-=column
 * 
 * Examples with use cases - Managment table with progress bar - Example with formulas
 * inserir rows and columns ta dando pala nas borders de highlight
 * quando a ultima coluna ou primiera escondida da problema
 * ao adicionar nova coluna ta dando problema nas borders. escondendo o hightlight
 *  * 
 *   examples:
 *   Spreadshhet project management
 *   Spreadsheet invetory food 
 */
...
var jexcel = (function(el, options) {
    ...
    var defaults = {
        ...
        // About message
        about:"jExcel Pro Spreadsheet\nVersion 4.0.1\nAuthor: Paul Hodel <paul@jexcel.net>\nWebsite: https://jexcel.net/v4",
    };
    ...
    obj.createTable = function() {
        ...
        progress.classList.add('jexcel_progress_container');
        obj.progress = document.createElement('div');
        obj.progress.classList.add('jexcel_progress');
        obj.progress.style.display = 'none';
        progress.appendChild(obj.progress);
        ...
        if (obj.options.fullscreen == true) {
            ...
            if (obj.options.tableOverflow == true) {
                ...
                if (obj.options.tableOverflowResizable == true) {
                    obj.content.style.resize = 'both';
                }
            }
        }
    ...
    obj.getData = function(highlighted) {
        if (! highlighted) {
            var x1 = 0;
            var y1 = 0;
            var x2 = obj.headers.length;
            var y2 = obj.rows.length;
        } else {
            var highlighted = obj.getSelected();

            // Matrix to be returned
            var x1 = highlighted[0].x;
            var y1 = highlighted[0].y;
            var x2 = highlighted[highlighted.length-1].x;
            var y2 = highlighted[highlighted.length-1].y;
        }
        ...
        for (var j = y1; j <= y2; j++) {
            var cols = [];
            for (var i = x1; i <= x2; i++) {
                if (obj.records[j][i].element && obj.options.copyCompatibility == true) {
                    cols.push(obj.recores[j][i].element.innerHTML);
                } else {
                    cols.push(obj.options.data[j][i]);
                }
            }
            rows.push(cols);
        }
    ...
    obj.getJson = function(highlighted) {
        ...
        for (var j = y1; j <= y2; j++) {
            var row = {};
            for (var i = x1; i < x2; i++) {
                if (obj.records[y][x].element && obj.options.copyCompatibility == true) {
                    row[obj.options.columns[i].name] = obj.recores[j][i].element.innerHTML;
                } else {
    ...
    obj.createRow = function(j, data) {
        ...
        // Data columns
        for (i = 0; i < obj.options.columns.length; i++) {
            var td = obj.createCell(i, j, data[i]);
            ...
            if (toolbar[i].type == 'i') {
                ...
                if (toolbar[i].onclick && typeof(toolbar[i].onclick)) {
                    ...
                    toolbarItem.onclick = function() {
                        ...
                        obj.setStyle(obj.getSelected(), k, v);
                    }
                }
                // Append element
               ...
               if (toolbar[i].onchange && typeof(toolbar[i].onchange)) {
                   ...
                   toolbarItem.onchange = function() {
                       var k = this.getAttribute('data-k');
                       obj.setStyle(obj.getSelected(), k, this.value);
                   }
               }
    ...
    obj.setMerge = function(cellName, colspan, rowspan, ignoreHistoryAndEvents) {
        if (obj.options.lazyLoading == true) {
            console.log('Merged cells are not available with lazyloading');
            return false;
        } else if (obj.options.pagination) {
            console.log('Merged cells are not available with pagination');
            return false;
        } else if (obj.options.search == true) {
            console.log('Merged cells are not available with search mode active');
            return false;
        }
        ...
        if (! cellName) {
            ...
            if (! highlighted.length) {
                ...
                var x1 = parseInt(highlighted[0].x);
                var y1 = parseInt(highlighted[0].y);
                var x2 = parseInt(highlighted[highlighted.length-1].x);
                var y2 = parseInt(highlighted[highlighted.length-1].y);
        ...
        if (test) {
            ...
            obj.updateSelection(obj.records[cell[1]][cell[0]].element);
    ...
    obj.removeMerge = function(cellName, data, keepOptions) {
        if (obj.options.mergeCells[cellName]) {
            ...
            // Update selection
            obj.updateSelectionFromCoords(obj.records[cell[1]][cell[0]].x, obj.records[cell[1]+j-1][cell[0]+i-1].y);
    ...
    obj.openEditor = function(cell, empty, e) {
        ...
        if (cell.classList.contains('readonly') == true) {
            ...
            // Holder
            obj.edition = [ cell, cell.innerHTML, x, y ];
    ...
    obj.getValue = function(cell, processedValue) {
        ...
        if (x != null && y != null) {
            if (obj.records[y][x].element && (processedValue || obj.options.copyCompatibility == true)) {
                ...
                if (keys.length > 0) {
                    for (var i = 0; i < keys.length; i++) {
                        if (typeof(cell[i]) == 'string') {
                            ...
                            if (cell[i].col != null && cell[i].row != null) {
                                var x = cell[i].col;
                                var y = cell[i].row;
                                // Flexible setup
    ...
    obj.updateCell = function(x, y, value, force) {
        ...
        if (! force && obj.options.columns[x].readOnly == true && (obj.records[y][x].element && obj.records[y][x].classList.contains('readonly') == true)) {
            // Do nothing
        } else {
            // On change
            if (! obj.ignoreEvents) {
                if (typeof(obj.options.onbeforechange) == 'function') {
                    // Overwrite a value
                    var val = obj.dispatch('onbeforechange', el, obj.records[y][x].element, x, y, value);
            ...
            var record = {
                ...
                row: y,
                newValue: value,
                oldValue: obj.options.data[y] && obj.options.data[y][x] ? obj.options.data[y][x] : '',
            }

            // Create row if does not exists
            ...
            if (obj.options.columns[x].editor) {
                ...
                obj.options.columns[x].editor.setValue(obj.records[y][x].element, value, force);
            } else {
                // Native functions
                if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                    ...
                    value = (value == 1 || value == true || value == 'true' || value == 'TRUE') ? true : false;

                    obj.options.data[y][x] = value;
                    if (obj.records[y][x].element) {
                        obj.records[y][x].element.children[0].checked = value;
                    }
                } else if (obj.options.columns[x].type == 'dropdown' || obj.options.columns[x].type == 'autocomplete') {
                    // Update data and cell
                    obj.options.data[y][x] = value;
                    if (obj.records[y][x].element) {
                        obj.records[y][x].element.innerHTML = obj.getDropDownValue(x, value);
                    }
                } else if (obj.options.columns[x].type == 'calendar') {
                    // Update calendar
                    ...
                    obj.options.data[y][x] = value;
                    if (obj.records[y][x].element) {
                        obj.records[y][x].element.innerHTML = jSuites.calendar.getDateString(formatted ? formatted : value, obj.options.columns[x].options.format);
                    }
                } else if (obj.options.columns[x].type == 'color') {
                    // Update color
                    obj.options.data[y][x] = value;
                    if (obj.records[y][x].element) {
                        if (obj.options.columns[x].render == 'square') {
                            ...
                            obj.records[y][x].element.innerHTML = value;
                        }
                    }
                } else if (obj.options.columns[x].type == 'image') {
                    value = ''+value;
                    ...
                    if (obj.records[y][x].element) {
                        ...
                        if (obj.options.columns[x].wordWrap != false && (obj.options.wordWrap == true || obj.options.columns[x].wordWrap == true || obj.records[y][x].element.innerHTML.length > 200)) {
            ...
            // On change
            obj.dispatch('onchange', obj.records[y][x].element, x, y, value, record.oldValue);
        }
     ...
     */
    obj.parseValue = function(x, y, value) {
        // Changing value depending on the column type
        if (obj.options.columns[x].readOnly == true && obj.records[y][x].element && obj.records[y][x].element.classList.contains('readonly') == true) {
            // Do nothing
        } else {
            if (obj.options.columns[x].editor) {
                // TODO: custom value
                ...
                if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                    ...
                    value = obj.getDropDownValue(x, value);
                } else if (obj.options.columns[x].type == 'calendar') {
                    var formatted = jSuites.calendar.extractDateFromString(value, obj.options.columns[x].options.format);
                    value = jSuites.calendar.getDateString(formatted ? formatted : value, obj.options.columns[x].options.format);
                } else if (obj.options.columns[x].type == 'color') {
                } else if (obj.options.columns[x].type == 'image') {
                } else {
                    if (('' + value).substr(0,1) == '='  && obj.options.parseFormulas == true) {
     ...
     */
    obj.copyData = function(x1, y1, x2, y2) {
        ...
        for (j = y1; j <= y2; j++) {
            ...
            for (var i = x1; i <= x2; i++) {
                ...
                if (breakControl == false) {
                    if (! (obj.records[j][i].element && ! obj.records[j][i].element.classList.contains('readonly'))) {
                        continue;
                    }
    ...
    obj.setBorder = function(x1, y1, x2, y2, border) {
        ...
        // Get last cell
        var first = obj.records[x1][y1].element.getBoundingClientRect();
        var x1 = first.left;
        var y1 = first.top;

        var last = obj.records[x2][y2].element.getBoundingClientRect();
    ...
    obj.resetSelection = function(blur) {
        ...
        if (! highlighted.length) {
            ...
            for (var i = 0; i < highlighted.length; i++) {
                if (highlighted[i].element) {
                    highlighted[i].element.classList.remove('highlight');
                    highlighted[i].element.classList.remove('highlight-left');
                    highlighted[i].element.classList.remove('highlight-right');
                    highlighted[i].element.classList.remove('highlight-top');
                    highlighted[i].element.classList.remove('highlight-bottom');
                    highlighted[i].element.classList.remove('highlight-selected');
                }

                var px = parseInt(highlighted[i].x);
                var py = parseInt(highlighted[i].y);

                // Check for merged cells
                if (highlighted[i].element && highlighted[i].element.getAttribute('data-merged')) {
                    var colspan = parseInt(highlighted[i].element.getAttribute('colspan'));
                    var rowspan = parseInt(highlighted[i].element.getAttribute('rowspan'));
    ...
    obj.updateSelectionFromCoords = function(x1, y1, x2, y2, origin) {
        ...
        if (x1 != null) {
            ...
            for (var i = px; i <= ux; i++) {
                for (var j = py; j <= uy; j++) {
                    if (obj.records[j][i].element && obj.records[j][i].element.getAttribute('data-merged')) {
                        var x = parseInt(obj.records[j][i].x);
                        var y = parseInt(obj.records[j][i].y);
            ...
            for (var i = px; i <= ux; i++) {
                for (var j = py; j <= uy; j++) {
                    if (obj.rows[j] && obj.records[j][i].element && obj.records[j][i].element.style.display != 'none') {
            ...
            for (var i = borderLeft; i <= borderRight; i++) {
                if (obj.options.columns[i].type != 'hidden') {
                    // Top border
                    if (obj.records[borderTop][i].element) {
                        obj.records[borderTop][i].element.classList.add('highlight-top');
                    }
                    // Bottom border
                    if (obj.records[borderBottom][i].element) {
        ...
        // Redefine selected cells
        obj.highlighted = [ borderLeft, borderTop, borderRight, borderBottom ];
    ...
    obj.updateCopySelection = function(x3, y3) {
        ...
        if (x3 && y3) {
            ...
            for (var j = py; j <= uy; j++) {
                for (var i = px; i <= ux; i++) {
                    if (obj.rows[j].style.display != 'none' && obj.records[j][i].element && obj.records[j][i].element.style.display != 'none') {
    ...
    obj.updateCornerPosition = function() {
        ...
        if (! highlighted.length) {
            ...
            // Get last cell
            var last = highlighted[highlighted.length-1].element;
    ...
    obj.updateScroll = function(direction) {
        ...
        if (direction == 0 || direction == 1) {
            ...
            var y = (y2 - y1) + obj.content.scrollTop;
        } else {
        ...
        if (y > (obj.content.scrollTop + 30) && y < (obj.content.scrollTop + h1)) {
            ...
            // Out of viewport
            if (y < obj.content.scrollTop + h2) {
                obj.content.scrollTop = y - (h2 + 2);
            } else {
        ...
        if (x > (obj.content.scrollLeft) && x < (obj.content.scrollLeft + w1)) {
            ...
            // Out of viewport
            if (x < obj.content.scrollLeft) {
                obj.content.scrollLeft = x;
                if (obj.content.scrollLeft < 50) {
                    obj.content.scrollLeft = 0;
                }
            } else {
                obj.content.scrollLeft = x - w1 + 20; // 20 is the size of the scrollbar
            }
        }
    }
    ...
    obj.resetStyle = function(o, ignoreHistoryAndEvents) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (obj.records[cell[1]][cell[0]].element) {
    ...
    obj.getComments = function(cell, withAuthor) {
        ...
        if (withAuthor) {
            return [obj.records[cell[1]][cell[0]].element.getAttribute('title'), obj.records[cell[1]][cell[0]].element.getAttribute('author')];
        } else {
    ...
    obj.setComments = function(cellId, comments, author) {
        ...
        // Keep old value
        var title = obj.records[cell[1]][cell[0]].element.getAttribute('title');
        var author = obj.records[cell[1]][cell[0]].element.getAttribute('data-author');
        var oldValue = [ title, author ];

        // Set new values
        obj.records[cell[1]][cell[0]].element.setAttribute('title', comments ? comments : '');
        obj.records[cell[1]][cell[0]].element.setAttribute('data-author', author ? author : '');

        // Remove class if there is no comment
        if (comments) {
            obj.records[cell[1]][cell[0]].element.classList.add('jexcel_comments');
        } else {
            obj.records[cell[1]][cell[0]].element.classList.remove('jexcel_comments');
        }
    ...
    obj.orderBy = function(column, order) {
        if (column >= 0) {
            ...
            if (obj.options.columns[column].type == 'calendar' ||
                ...
                for (var j = 0; j < obj.options.data.length; j++) {
                    temp[j] = [ j, obj.records[j][column].element ? obj.records[j][column].element.innerHTML : obj.options.data[j][column] ];
                }
            }
    ...
    obj.updateOrder = function(rows) {
        ...
        if (obj.searchInput.value) {
            ...
            if (obj.options.lazyLoading == true || obj.options.pagination > 0) {
                obj.page(0);
            } else {
                for (var j = 0; j < obj.rows.length; j++) {
                    obj.appendRow(j);
    ...
    obj.insertRow = function(mixed, rowNumber, insertBefore) {
        ...
        if (obj.options.allowInsertRow == true) {
            ...
            if (typeof(obj.options.onbeforeinsertrow) == 'function') {
                if (! obj.dispatch('onbeforeinsertrow', el, rowNumber, numOfRows, insertBefore)) {
            ...
            // Reselect
            if (selection) {
                obj.updateSelectionFromCoords(selection[0], selection[1], selection[2], selection[3]);
            }
        ...
        if (obj.options.allowDeleteRow == true) {
            if (obj.options.allowDeletingAllRows == true || obj.options.data.length > 1) {
                ...
                if (typeof(obj.options.onbeforedeleterow) == 'function') {
                    if (! obj.dispatch('onbeforedeleterow', el, rowNumber, numOfRows)) {
                ...
                if (parseInt(rowNumber) > -1) {
                    ...
                    for (var row = rowNumber; row < rowNumber + numOfRows; row++) {
                        if (Array.prototype.indexOf.call(obj.tbody.children, obj.rows[row]) >= 0) {
                            for (var col = 0; col < obj.headers.length; col++) {
    ...
    obj.moveColumn = function(o, d) {
        ...
        if (o > d) {
            ...
            for (var j = 0; j < obj.rows.length; j++) {
                obj.rows[j].insertBefore(obj.records[j][o].element, obj.records[j][d].element);
            }
        } else {
            ...
            for (var j = 0; j < obj.rows.length; j++) {
                obj.rows[j].insertBefore(obj.records[j][o].element, obj.records[j][d].element.nextSibling);
            }
        }
    ...
    obj.insertColumn = function(mixed, columnNumber, insertBefore, properties) {
        ...
        if (obj.options.allowInsertColumn == true) {
            ...
            if (typeof(obj.options.onbeforeinsertcolumn) == 'function') {
                if (! obj.dispatch('onbeforeinsertcolumn', el, columnNumber, numOfColumns, insertBefore)) {
            ...
            // Events
            obj.dispatch('oninsertcolumn', el, columnNumber, numOfColumns);
        }
    }
        ...
        if (obj.options.allowDeleteColumn == true) {
            if (obj.headers.length > 1) {
                ...
                if (typeof(obj.options.onbeforedeletecolumn) == 'function') {
                   if (! obj.dispatch('onbeforedeletecolumn', el, columnNumber, numOfColumns)) {
                ...
                if (parseInt(columnNumber) > -1) {
                    ...
                    for (var row = 0; row < obj.options.data.length; row++) {
                        for (var col = columnNumber; col < columnNumber + numOfColumns; col++) {
                            if (obj.records[row][col].element) {
                                obj.records[row][col].element.className = '';
    ...
    obj.isSelected = function(x, y) {
        // Get coordinates
        var x1 = obj.selectedCell[0];
        var y1 = obj.selectedCell[1];
        var x2 = obj.selectedCell[2];
        var y2 = obj.selectedCell[3];
    ...
    obj.getSelected = function(asIds) {
        var cells = [];
        if (obj.highlighted) {
            // Get coordinates
            if (parseInt(obj.highlighted[0]) <= parseInt(obj.highlighted[2])) {
                var x1 = parseInt(obj.highlighted[0]);
                var x2 = parseInt(obj.highlighted[2]);
            } else {
                var x1 = parseInt(obj.highlighted[2]);
                var x2 = parseInt(obj.highlighted[0]);
            }

            if (parseInt(obj.highlighted[1]) <= parseInt(obj.highlighted[3])) {
                var y1 = parseInt(obj.highlighted[1]);
                var y2 = parseInt(obj.highlighted[3]);
            } else {
                var y1 = parseInt(obj.highlighted[3]);
                var y2 = parseInt(obj.highlighted[1]);
            }
            ...
            for (var j = y1; j <= y2; j++) {
                for (var i = x1; i <= x2; i++) {
                    if (asIds) {
                        cells.push(jexcel.getColumnNameFromId([i, j]));
                    } else {
                        cells.push(obj.records[j][i]);
                    }
                }
            }
        }
    ...
    obj.updateTableReferences = function() {
        ...
        for (var j = 0; j < obj.records.length; j++) {
            for (var i = 0; i < obj.headers.length; i++) {
                // Current values
                var x = obj.records[j][i].x;
                var y = obj.records[j][i].y;
            ...
            for (var i = 0; i < keys.length; i++) {
                if (mergeCellUpdates[keys[i]]) {
                    ...
                    for (var j = 0; j < obj.options.mergeCells[columnIdFrom][2].length; j++) {
                        var x = parseInt(obj.options.mergeCells[columnIdFrom][2][j].x);
                        var y = parseInt(obj.options.mergeCells[columnIdFrom][2][j].y);
                        obj.options.mergeCells[columnIdFrom][2][j].x = x + mergeCellUpdates[keys[i]][1];
                        obj.options.mergeCells[columnIdFrom][2][j].y = y + mergeCellUpdates[keys[i]][2];
                    }
    ...
    obj.updateTable = function() {
        ...
        if (typeof(obj.options.updateTable) == 'function') {
            for (var j = 0; j < obj.rows.length; j++) {
                for (var i = 0; i < obj.headers.length; i++) {
                    obj.options.updateTable(el, obj.records[j][i].element, i, j, obj.options.data[j][i], obj.records[j][i].element.innerText, jexcel.getColumnNameFromId([i, j]));
                }
            }
        }
        ...
        // TODO: improve this - Update corner position after render
        setTimeout(function() {
            obj.updateCornerPosition();
        }, 0);
    }
    ...
    obj.up = function(shiftKey, ctrlKey) {
        ...
        if (y > 0) {
            if (! ctrlKey) {
                var cell = null;
                if (obj.records[y][x].element) {
                    cell = obj.records[y][x].element.parentNode.previousSibling;

                    if (! cell) {
                        var currentPage = obj.whichPage(y);
                ...
                if (cell) {
                    ...
                    if (obj.options.lazyLoading == true || obj.options.pagination > 0) {
                        ...
                        if (currentPage > 0) {
                            ...
                            // Adjust scroll
                            obj.content.scrollTop = obj.content.scrollHeight;
                            // Get the first record from the next page
                            var cell = obj.tbody.lastChild.children[x + 1];
                        }
                    }
                }
            } else {
                ...
                // Get the first record in the first page
                var cell = obj.tbody.firstChild.children[x + 1];
            }
            ...
            if (cell) {
                if (shiftKey) {
                    ...
                    var x1 = cell.getAttribute('data-x');
                    var y1 = cell.getAttribute('data-y');
                    var x2 = x1;
                    var y2 = y1;
                }
    ...
    obj.down = function(shiftKey, ctrlKey) {
        ...
        if (y < obj.options.data.length - 1) {
            // Move cell
            if (! ctrlKey) {
                var cell = null;
                if (obj.records[y][x].element) {
                    cell = obj.records[y][x].element.parentNode.nextSibling;
                ...
                if (cell) {
                    ...
                    if (obj.options.lazyLoading == true || obj.options.pagination > 0) {
                        // Quantity of pages
                        var quantityOfPages = obj.quantityOfPages();
                        // Current page
                        var currentPage = obj.whichPage(y);
                ...
                if (obj.options.lazyLoading == true || obj.options.pagination > 0) {
                    ...
                    if (currentPage < quantityOfPages - 1) {
                        // Go to the last page
                        obj.page(quantityOfPages - 1);
                    }
                }
                // Get the last record in the last page
                var cell = obj.tbody.lastChild.children[x + 1];
            }
    ...
    obj.right = function(shiftKey, ctrlKey) {
        ...
        if (x < obj.headers.length) {
            if (! obj.rows[y] || ! obj.rows[y].parentNode) {
                var currentPage = obj.whichPage(y);
                obj.page(currentPage);
            }

            var children = obj.records[y][x].element.parentNode.children;

            if (! ctrlKey) {
                for (var i = x + 2; i < children.length; i++) {
                ...
                for (var i = children.length - 1; i >= x; i--) {
                    if (children[i].style.display != 'none') {
                        var cell = children[i];
                        break;
                    }
    ...
    obj.first = function(shiftKey, ctrlKey) {
        if (ctrlKey) {
            obj.up(shiftKey, true)
        } else {
            obj.left(shiftKey, true)
        }
    }

    obj.last = function(shiftKey, ctrlKey) {
        if (ctrlKey) {
            obj.down(shiftKey, true)
        } else {
            obj.right(shiftKey, true)
        }
    }
    ...
    obj.search = function(query) {
        ...
        if (query) {
            ...
            var search = function(item, query, index) {
                for (var i = 0; i < item.length; i++) {
                    var k = (''+item[i]).toLowerCase();
                    var v = obj.records[index][i].element ? obj.records[index][i].element.innerHTML : '';

                    if (k.search(query) >= 0 || v.search(query) >= 0) {
                        return true;
                    }
                }
                return false;
        ...
        // Total records to be returned
        if (obj.options.lazyLoading == true && total > 50) {
            total = 50;
        } else if (obj.options.pagination > 0 && total > obj.options.pagination) {
        ...
        for (var j = 0; j < total; j++) {
            if (obj.results) {
                var record = obj.results[j];
            } else {
                var record = j;
            }

            obj.appendRow(record);
        }
    ...
    obj.whichPage = function(cell) {
        if (obj.options.search == true && obj.results) {
            cell = obj.results.indexOf(parseInt(cell));
        }
        ...
        if (obj.options.lazyLoading == true) {
            ...
            if (obj.options.pagination > 0) {
                var pagination = obj.options.pagination;
            }
        }

        if (! pagination) {
            console.log('JEXCEL: No pagination defined');
            return false;
        }

        return (Math.ceil((parseInt(cell) + 1) / parseInt(pagination))) - 1;
    }

    obj.quantityOfPages = function() {
        // Searchable
        if (obj.options.search == true && obj.results) {
        ...
        return Math.ceil(results / pagination);
    }
    ...
    obj.page = function(pageNumber) {
        ...
        // Per page
        if (obj.options.lazyLoading == true) {
            var quantityPerPage = 50;
        } else {
            var quantityPerPage = parseInt(obj.options.pagination);
        }

        if (! quantityPerPage) {
            console.error('JEXCEL pagination not defined');
        } else {
            // pageNumber
            ...
            for (var j = startRow; j < finalRow; j++) {
                ...
                // Append node
                obj.tbody.appendChild(obj.rows[record]);
            }
            ...
            if (item > 0) {
                for (var j = 0; j < 50; j++) {
                    ...
                    if (item > -1) {
                        ...
                        obj.tbody.insertBefore(obj.rows[record], obj.tbody.firstChild);
    ...
    obj.copy = function(highlighted, delimiter, returnData) {
        ...
        for (var j = 0; j < y; j++) {
            ...
            for (var i = 0; i < x; i++) {
                ...
                if (! highlighted || obj.records[j][i].element && obj.records[j][i].element.classList.contains('highlight')) {
                    // Values
                    var value = obj.options.data[j][i];
                    ...
                    // Labels
                    var label = obj.records[j][i].element ? obj.records[j][i].element.innerHTML : obj.options.data[j][i];
    ...
    obj.paste = function(x, y, data) {
        ...
        if (typeof(obj.options.onbeforepaste) == 'function') {
            var data = obj.dispatch('onbeforepaste', el, data);
        }
        ...
        if (x != null && y != null && data) {
            ...
            // Go through the columns to get the data
            while (row = data[j]) {
                // There is no row in the DOM
                if (! obj.rows[rowIndex]) {
                    if (! obj.options.lazyLoading && ! obj.options.pagination) {
                        obj.insertRow();
                    } else {
                        // Create blank row on memory
                        if (! obj.options.data[rowIndex]) {
                            obj.rows[rowIndex] = '';
                            obj.records[rowIndex] = [];
                            obj.options.data[rowIndex] = [];
                            for (var i = 0; i < obj.headers.length; i++) {
                                obj.options.data[rowIndex][i] = '';
                            }
                        }
                    }
                }
                ...
                while (row[i] != null) {
                    // Create a non existing column
                    if (! obj.headers[colIndex]) {
                        obj.insertColumn();
                    }
            ...
            obj.updateSelectionFromCoords(x, y, colIndex - 1, rowIndex - 1);
            ...
            // Paste event
            obj.dispatch('onpaste', el, records);
    ...
    obj.historyProcessRow = function(type, historyRecord) {
        ...
        if (type == 1) {
            ...
            for (var j = rowIndex; j < (numOfRows + rowIndex); j++) {
                for (var i = 0; i < obj.headers.length; i++) {
                    obj.records[j][i].element.className = '';
                }
                obj.rows[j].className = '';
        ...
        if (obj.historyIndex >= 0) {
            ...
            if (historyRecord.action == 'insertRow') {
                ...
                for (var j = 0; j < historyRecord.rows.length; j++) {
                    records[historyRecord.rows[j]] = j;
                }
                obj.updateOrder(records);
                ...
                for (var i = 0; i < historyRecord.records.length; i++) {
                    records.push({
                        col: historyRecord.records[i].col,
                        row: historyRecord.records[i].row,
    ...
    obj.setProgress = function(val) {
        obj.progress.style.display = val == true ? '' : 'none';
    }
    ...
    obj.dispatch = function(event) {
        if (! obj.ignoreEvents) {
            ...
            if (obj.options.persistance == true) {
            }
            ...
            if (y == null) {
                ...
                if (x) {
                    if (obj.options.allowComments == true) {
                        ...
                        var title = obj.records[y][x].element.getAttribute('title') || '';
    ...
    obj.scrollLoading = function() {
        if (obj.options.lazyLoading == true) {
            if (jexcel.timeControlLoading == null) {
                if (obj.content.scrollTop + (obj.content.clientHeight * 2) >= obj.content.scrollHeight) {
                    if (obj.loadDown()) {
                        obj.updateCornerPosition();
                        obj.refreshSelection();
                    }
    ...
    obj.updateFreezePosition = function() {
        ...
        if (scrollLeft > 50) {
            for (var i = 0; i < obj.options.freezeColumns; i++) {
                ...
                obj.headers[i].style.left = (i > 0 ? obj.options.columns[i-1].width : 0) + 'px';

                for (var j = 0; j < obj.rows.length; j++) {
                    if (obj.records[j][i].element) {
                        var shifted = (scrollLeft + (i > 0 ? obj.records[j][i-1].element.style.width : 0)) - 51 + 'px';
                        if (shifted > scrollLeft - obj.content.style.width);
                        obj.records[j][i].element.classList.add('jexcel_freezed');
            ...
            for (var i = 0; i < obj.options.freezeColumns; i++) {
                ...
                for (var j = 0; j < obj.rows.length; j++) {
                    if (obj.records[j][i].element) {
                        obj.records[j][i].element.classList.remove('jexcel_freezed');
...
jexcel.destroy = function(element, destroyEventHandlers) {
    if (element.jexcel) {
        ...
        if (destroyEventHandlers) {
            document.removeEventListener("wheel", jexcel.wheelControls);
...
jexcel.build = function() {
    document.addEventListener("wheel", jexcel.wheelControls);
 ...
 * Events
 */
jexcel.wheelControls = function(e) {
    if (jexcel.current) {
        jexcel.current.scrollLoading();
        if (jexcel.current.options.pagination > 0) {
            var page = jexcel.current.whichPage(jexcel.current.tbody.firstChild.getAttribute('data-y'));
            if (e.deltaY < 0) {
                if (page > 0) {
                    jexcel.current.page(page-1);
                }
            } else {
                if (page < jexcel.current.quantityOfPages() - 1) {
                    jexcel.current.page(page+1);
                }
            }
        }
        ...
        if (! jexcel.current.edition && jexcel.current.selectedCell) {
            ...
            if (e.which == 37) {
                ...
                if (jexcel.current.options.editable == true) {
                    if (jexcel.current.selectedRow) {
                        ...
                        // Change value
                        jexcel.current.setValue(jexcel.current.getSelected(true), '');
                    }
                }
            } else if (e.which == 13) {
                // Move cursor
                ...
                if (e.shiftKey) {
                    ...
                    var column = jexcel.current.selectedCell[0];
                    ...
                    if (jexcel.current.options.allowInsertColumn == true) {
                        if (jexcel.current.options.allowManualInsertColumn == true) {
                            if (jexcel.current.selectedCell[0] == column) {
                            ...
                            if (jexcel.current.options.columns[columnId].type != 'readonly') {
                                ...
                                if (e.keyCode == 32) {
                                    ...
                                    if (jexcel.current.options.columns[columnId].type == 'checkbox' ||
                                        ...
                                        // Start edition
                                        jexcel.current.openEditor(jexcel.current.records[rowId][columnId].element, true);
                                    }
                                } else if (e.keyCode == 113) {
                                    // Start edition with current content F2
                                    jexcel.current.openEditor(jexcel.current.records[rowId][columnId].element, false);
                                } else if ((e.keyCode == 8) ||
            ...
            if (e.target.classList.contains('jexcel_search')) {
                ...
                jexcel.timeControl = setTimeout(function() {
                    jexcel.currentSearch = e.target.value;
                ...
                if (columnId) {
                    ...
                    if (jexcel.current.options.columnResize == true && info.width - e.offsetX < 6) {
                        ...
                        for (var j = 0; j < jexcel.current.records.length; j++) {
                            if (jexcel.current.records[j][columnId].element) {
                                jexcel.current.records[j][columnId].element.classList.add('resizing');
                            }
                        }
                        ...
                        if (jexcel.current.isColMerged(columnId).length) {
                            ...
                            for (var j = 0; j < jexcel.current.records.length; j++) {
                                if (jexcel.current.records[j][columnId].element) {
                                    jexcel.current.records[j][columnId].element.classList.add('dragging');
                                }
                            }
                        }
                    } else {
    ...
    if (jexcel.current) {
        ...
        if (jexcel.current.resizing) {
            ...
            if (jexcel.current.resizing.column) {
                ...
                if (columns.length > 1 && 1 == 2) { // TODO: undo/redo for multiple columns
                    for (var i = 0; i < columns.length; i++) {
                        var currentWidth = parseInt(jexcel.current.colgroup[columns[i]].getAttribute('width'));
                        jexcel.current.setWidth(columns[i], newWidth, currentWidth);
                    }
                } else {
                ...
                for (var j = 0; j < jexcel.current.records.length; j++) {
                    if (jexcel.current.records[j][jexcel.current.resizing.column].element) {
                        jexcel.current.records[j][jexcel.current.resizing.column].element.classList.remove('resizing');
                    }
                }
                // Update frezee position
                if (jexcel.current.options.freezeColumns > 0) {
            ...
            if (jexcel.current.dragging) {
                if (jexcel.current.dragging.column) {
                    ...
                    for (var j = 0; j < jexcel.current.rows.length; j++) {
                        if (jexcel.current.records[j][jexcel.current.dragging.column].element) {
                            jexcel.current.records[j][jexcel.current.dragging.column].element.classList.remove('dragging');
                        }
                    }
            ...
            if (jexcel.current.selectedCorner) {
                ...
                if (jexcel.current.selection.length) {
                    ...
                    var x1 = jexcel.current.selection[0].getAttribute('data-x');
                    var y1 = jexcel.current.selection[0].getAttribute('data-y');
                    var x2 = jexcel.current.selection[jexcel.current.selection.length-1].getAttribute('data-x');
                    var y2 = jexcel.current.selection[jexcel.current.selection.length-1].getAttribute('data-y');
                    // Copy data
                    jexcel.current.copyData(x1, y1, x2, y2);
                    // Remove selection
...
jexcel.doubleClickControls = function(e) {
    ...
    if (jexcel.current) {
        ...
        if (e.target.classList.contains('jexcel_corner')) {
            var highlighted = jexcel.current.getSelected();
            // Any selected cells
            if (highlighted.length > 0) {
                // Update those coordinates
                var x1 = highlighted[0].x;
                var y1 = parseInt(highlighted[highlighted.length - 1].y) + 1;
                var x2 = highlighted[highlighted.length - 1].x;
                var y2 = jexcel.current.options.data.length - 1;
                // Execute copy
                jexcel.current.copyData(x1, y1, x2, y2);
            }
        } else {
...
jexcel.cutControls = function(e) {
    if (jexcel.current) {
        if (! jexcel.current.edition) {
            jexcel.current.copy(true);
            if (jexcel.current.options.editable == true) {
                jexcel.current.setValue(jexcel.current.getSelected(), '');
            }
        }
    }
}
